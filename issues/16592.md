...
defines how canonical changes are observed

```reth/crates/evm/execution-types/src/chain.rs```
gives those notifications rich block/receipt data

```reth/crates/rpc/rpc-eth-types/src/logs_utils.rs```
transforms that data into RPC Logs

```reth/crates/rpc/rpc/src/eth/pubsub.rs``` 
wires everything into JSON-RPC subscriptions so clients receive heads/logs/pending-tx updates backed by the canonical state
